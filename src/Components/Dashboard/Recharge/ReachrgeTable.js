...
const ReachrgeTable = ({ reachrges }) => {
  ...
  return (
    <tbody class="divide-y divide-gray-100 border-t border-gray-100">
      {reachrges?.map(
        ({ PayId, images, email, RechargeAmoun, _id, status }) => (
          <tr class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4">
              <div class="flex ">
                <span
                  onClick={() => showImageHendler(images?.url)}
                  class={`bg-red-50 text-[#ff4019] inline-flex items-center gap-1 rounded-full  px-2 py-1 text-xs font-semibold`}
                >
                  Show Image
                </span>
            ...
            <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
              <span
                onClick={() => reachrgeHendler(_id, email, RechargeAmoun)}
